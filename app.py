...
# ── Session state init (WAJIB sebelum apapun) ─────────────────────────────────
...
# ── Sidebar (render dulu pakai session_state) ─────────────────────────────────
with side_col:
...
def _alert(bad, active):
    if not active:
        return '<div class="alert-ok" style="opacity:.5">Menunggu kamera…</div>'
    if bad > 0:
...
class VideoProcessor(VideoProcessorBase):
    ...
    def recv(self, frame):
        ...
        if bad_count > 0:
            ov2 = img.copy()
            cv2.rectangle(ov2, (0, 0), (orig_w, orig_h), (68, 68, 239), -1)
            cv2.addWeighted(ov2, 0.08, img, 0.92, 0, img)
        ...
        # Simpan ke session_state (thread-safe via Streamlit)
        st.session_state.good_count = good_count
        st.session_state.bad_count  = bad_count
...
# ── Update sidebar dari session_state (aman, tidak trigger rerun) ─────────────
is_active = ctx.state.playing if ctx.state else False

good = st.session_state.good_count
bad  = st.session_state.bad_count
...
alert_ph.markdown(_alert(bad, is_active), unsafe_allow_html=True)